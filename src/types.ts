...
export type Project = { id:string; name:string; createdAt:number; updatedAt:number; image?:string; imageWidth?:number; imageHeight?:number; controlPoints:ControlPoint[]; checkpoints:Checkpoint[]; startPointId?:string; endPointId?:string; base?:LatLng; includeBaseStart?:boolean; includeBaseEnd?:boolean; route?:RouteResult; routeVariants?:RouteVariant[]; selectedRouteVariant?:number; opacity:number; imageTransform:{x:number;y:number;scale:number;rotation:number} };
